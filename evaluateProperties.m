...
%% This function evaluates and returns various thermophysical properties required to define the derivates of the variables being solved (for) by the ODE solver, ODE45.

function [rhoLiqOne, dRhoBydTempOne, rhoLiqTwo, dRhoBydTempTwo, rhoLiq, rhoVapFilm, binDiffusivityOne, binDiffusivityTwo, ShStarCompOne, ShStarCompTwo, BmCompOne, BmCompTwo, Bt, cpVapor, condVapFilm, mixEnthalpy, cpLiq, Re, rhoInf, Nu0, NuStaar] = evaluateProperties(dropSurfTemp, dropDia, compTwoLiqMaFr, relativeVel)
...
if mikhil == 1 % Suitable for single and bi-component liquids !
    ...
    if strcmp(componentTwoDepletionCalculationMethod,'usingVaporPressure') == 1 || (strcmp(componentTwoDepletionCalculationMethod,'usingArrheniusEquation') == 1 && dropSurfTemp < compOneBoilingPoint ) %compTwoLiqMaFr < 1 ) 
