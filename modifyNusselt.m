...
%% This function modifies the Nusselt number using.  
%  Can be modified to use other expressions for modifying Nu
%  Inputs and outputs are in SI units

function [NuStaar] = modifyNusselt(Nu, Bt, Reynolds, calculationMethod)

%% select the correlation
mikhil = strcmp(calculationMethod, 'mikhil');

%% modify Nusselt number

if mikhil == 1 
    
    if Bt > 0 
    
        fBT = (((1+Bt)^0.7) * log(1+Bt)) / Bt ;
