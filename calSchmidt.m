...
%% This function calculates the schmidt number.  
%  Inputs and outputs are in SI units

function [Schmidt] = calSchmidt(mu, rho, Dab,ambientTemperature, surfaceTemperature, I, vapor, writeData, calMethod)
...
if mikhil == 1
    ...
    if ureaVapor == 1
        ...
        if saveDataToFile == 1
            ...
            filename = [savePath,'schmidtUreaVapor','_',num2str(ambientTemperature),'K.txt'];
        ...
        if saveDataToFile == 1
            ...
            if waterVapor == 1 
                filename = [savePath,'schmidtWaterVapor','_',num2str(ambientTemperature),'K.txt'];
        
            elseif nHeptaneVapor == 1
                filename = [savePath,'schmidtHeptaneVapor','_',num2str(ambientTemperature),'K.txt'];
        
            elseif nDecaneVapor == 1
                filename = [savePath,'schmidtDecaneVapor','_',num2str(ambientTemperature),'K.txt'];
                
            elseif vaporMixture == 1
                filename = [savePath,'schmidtVaporMixture','_',num2str(ambientTemperature),'K.txt'];
