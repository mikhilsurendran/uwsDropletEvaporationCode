...
%% This function defines the events function for ODE45

function [check, isterminal, direction] = eventsFunction(~,y) 

inputFile;

direction = []; % default condtions for direction

isterminal = 1; % ODE function will stop when the specified event has occured. 

check = double ( ( y(2) > 1e-6 ));  % Events that decide the termination of the ODE function
