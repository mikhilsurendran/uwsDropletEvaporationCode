...
%% This function calculates the Nusselt number for the flow around the droplet.  
%  Inputs and outputs are in SI units

function [Nusselt] = calNusselt(Reynolds, Grashof, Prandtl, ambientTemperature, I, writeData, calMethod)
...
if mikhil == 1 || frossling == 1 
    ...
    if Reynolds == 0
        ...
        Nusselt = 2 + 0.591 * Grashof^0.25 * Prandtl^(1/3) ;            % Correlation by Daif et al.
        
    else
        
        Nusselt = 2 + 0.552 * Reynolds^0.5 * Prandtl^(1/3) ;            % Correlation by Frossling et al.
...
if renksizbulut == 1   
    ...
    if Reynolds == 0
        ...
        Nusselt = 2 + 0.57 * Reynolds^0.5 * Prandtl^(1/3) ;             % Correlation by Renksizbulut et al.
...
if ranzMarshall == 1
    ...
    if Reynolds == 0
        ...
        Nusselt = 2 + 0.6 * Reynolds^0.5 * Prandtl^(1/3) ;              % Correlation by Ranz and Marshall.
...
if saveDataToFile == 1
    ...
    filename = [savePath,'nusselt','_',num2str(ambientTemperature),'K.txt'];
