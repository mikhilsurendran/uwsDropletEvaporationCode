...
%% This function calculates the Grashof number.  

function [Grashof] = calGrashof(density, mu, d, ambientTemperature, surfaceTemperature, I, writeData, calMethod)
...
if mikhil == 1
    ...
    filmTemperature = surfaceTemperature * (0.667) + ambientTemperature * (0.333) ; % using one third rule
    
    beta = 1 / filmTemperature ; 
    
    Grashof = 9.81 * beta * abs(surfaceTemperature - ambientTemperature) * d^3 / (mu/density)^2 ;
...
if saveDataToFile == 1
    ...
    filename = [savePath,'grashof','_',num2str(ambientTemperature),'K.txt'];
